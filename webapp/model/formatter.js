sap.ui.define([
	...
	var formatter = {
	   ...
	   */
	   ...
	   priceReverter: function (sValue) {
		
		if (sValue==="0,00"|| sValue===""){
			sValue = 0;
		}
		...
		if (!isNaN(value)){
		 ...
		 var valor = 0;	
		 sValue = sValue.toString().trim().replace(",","A");
		 sValue = sValue.toString().trim().replace(".","");
		 sValue = sValue.toString().trim().replace("A","."); 
		 
		 valor = parseFloat(sValue);

		return valor;
	  },

	  priceC: function (sValue) {
		...
		sValue = sValue.toString().trim().replace(",","");
		sValue = sValue.toString().trim().replace("A",","); 
		var ArrayV = sValue.split(",");
		if (ArrayV.length > 1){
			sValue = ArrayV[0] + "," + ArrayV[1].substring(0,2);
		}
	  ...
	  retiraZeroEsquerda: function(sValue){
		var valor = sValue.replace(/^(0+)(\d)/g,"$2");
		return valor;
	  },
