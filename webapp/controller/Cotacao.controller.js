sap.ui.define(
  ...
  function (
    ...
    return Controller.extend("com.assistente.controller.Cotacao", {
      formatter: formatter,

      onInit: function () {
        //this.byId("idValidoAte").setDateValue(new Date());
        //this.byId("idDataPedido").setDateValue(new Date());
      ...
      _onMasterMatched: function (oEvent) {
        ...
        if (oEvent.getParameters().name === "cotacao") {
          this.getView().setBusy(true);
          this.getMatchodes();
          ...
          if (oArgument.Id !== "0") {
            gDocumento = oArgument.Id;
            this.getView()
              .byId("idDadosCotacao")
              .setText("Dados da cotação " + gDocumento);
            ...
            stateView = "VISUALIZA";
            //Caso a cotação seja solicitada para alterar
            this.getView().setBusy(true);
            this.buscaDadosCotacao(oArgument.Id, "B");
          } else {
            stateView = "CRIA";
            this.getView().byId("idDadosCotacao").setText("Dados da cotação");
      ...
      getMatchodes: function () {
        ...
        this.getView().setBusy(true);
        Results.buscaTipoContacao().then(
          function (data) {
            var oData = {};
        ...
        Results.buscaMotivoRecusa().then(
          function (data) {
            ...
            var oModelSearch = new JSONModel();
            oData.Itens = data;
            oModelSearch.setSizeLimit(5000);
            oModelSearch.setData(oData);
      ...
      validaCentro: function () {
        ...
        if (vCount > 1) {
          MessageBox.alert("Centros diferentes");
          return true;
        }
      },
              ...
              Results.buscaPreco(
                ...
                function (data) {
                  ...
                  if (data.length === 1) {
                    ...
                    sPath1 = gPath + "/UM";
                    __this.getView().getModel("ModelItens").setProperty(sPath1, data[0].UnidadeMedida);
                  }
                },
                function (error) {}
              );
      ...
      onCalcular: function (oEvent) {
        ...
        Results.buscaDadosItens(dadosTela).then(
          function (data) {
            __this.getView().byId("idOK").setEnabled(true);
      ...
      onSalvar: function (oEvent) {
        var this_ = this;
        MessageBox.confirm("Confirma salvamento da cotação?", {
          title: "Informação",
          initialFocus: sap.m.MessageBox.Action.CANCEL,
          onClose: function (sButton) {
            if (sButton === MessageBox.Action.OK) {
      ...
      validaInformacao: function (oEvent) {
        ...
        if (stateView === "MODIFICA") {
          //Modifica Cotação
          Results.modificaCotacao(dados).then(
            function (data) {
              var mensagem = data[0].Mensagem;
              MessageBox.alert(mensagem);
              __this.getView().setBusy(false);
              //Limpa a tela em caso de sucesso
              if (mensagem.substr(0, 7) === "Cotação") {
          ...
          //Cria Cotação
          Results.criaCotacao(dados).then(
            function (data) {
              var mensagem = data[0].Mensagem;
              MessageBox.alert(mensagem);
              __this.getView().setBusy(false);
              ...
              if (mensagem.substr(0, 7) === "Cotação") {
                ...
                gDocumento = mensagem.substr(9, 10);
                __this
                  .getView()
                  .byId("idDadosCotacao")
                  .setText("Dados da cotação " + gDocumento);
        ...
        Results.buscaPreco(
          ...
          function (data) {
            ...
            if (data.length === 1) {
              ...
              sPath1 = sPath + "/UM";
              this_.getView().getModel("ModelItens").setProperty(sPath1, data[0].UnidadeMedida);
            }
          },
          function (error) {}
        );
      ...
      onCancelarCotacao: function () {
        var this_ = this;
        MessageBox.confirm("Deseja realmente cancelar a cotação ?", {
          title: "Informação",
          initialFocus: sap.m.MessageBox.Action.CANCEL,
          onClose: function (sButton) {
            if (sButton === MessageBox.Action.OK) {
      ...
      buscaDadosCotacao: function (sCotacao) {
        var __this = this;

        Results.buscaDadosCotacao(sCotacao, "B").then(
          function (data) {
            var dadosItens = [];
            var dadosAddVet = [];
            var count = 0;
            ...
            if (data[0].Mensagem !== "") {
              ...
              //Campos da tela
              __this.getView().byId("idTpCotacao").setValue(data[0].TpCotacao);
              ...
              __this
                ...
                .setValue(data[0].IndicacaoComercial);
              __this.getView().byId("idLocal").getValue(data[0].Local);
              __this
                .getView()
                .byId("idBloqueio")
                .setValue(data[0].BlockRemessa);
      ...
      stateLineItem: function (sStateView) {
        if (sStateView === "CRIA" || sStateView === "MODIFICA") {
      ...
      liveChangeRecebedor: function (oEvent) {
        ...
        var tipocli = "";
        sInputValue = this.getView().byId("idRecebedor").getValue();
          ...
          .read("/ZVND_CDS_C_RECMERC", {
            ...
            error: function (oResult) {
              ...
              if (reader.error.message.value.search("Parceiro") != -1) {
                MessageBox.error(
                  "Parceiro não se encontra nos canais do Cliente Principal"
                );
              } else {
